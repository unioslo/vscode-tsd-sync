...
export class TsdSyncTaskProvider implements vscode.TaskProvider {
  ...
  public async provideTasks(): Promise<vscode.Task[]> {
    console.log("tsdSyncTaskProvider", "provideTasks");
    return this.getTasks();
  }

  public resolveTask(_task: vscode.Task): vscode.Task | undefined {
    console.log("tsdSyncTaskProvider", "resolveTask");
  ...
  private getTask(
    ...
    if (definition === undefined) {
      definition = {
        type: TsdSyncTaskProvider.customBuildScriptType,
        flavor,
        flags,
      };
    }
    return new vscode.Task(
      ...
      `${flavor} ${flags.join(" ")}`,
      TsdSyncTaskProvider.customBuildScriptType,
...
class CustomBuildTaskTerminal implements vscode.Pseudoterminal {
  ...
  open(initialDimensions: vscode.TerminalDimensions | undefined): void {
    console.log("tsdSyncTaskProvider", "open");
    // At this point we can start using the terminal.
    if (this.flags.indexOf("watch") > -1) {
      const pattern = path.join(this.workspaceRoot, "*test*");
